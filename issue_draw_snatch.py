...
def SnatchDLTDataFrom500(lotType, lotNo, resultOnly = False):
    ...
    WURLFARMAT = 'http://kaijiang.500.com/%s.shtml?ts=' + str(time())
...
def SnatchQXCDataFrom500(lotType, lotNo, resultOnly = False):
    ...
    try:
        ...
        printExe("QXC some error occurred!")
        return(-1)               
...
def InsertQXCDrawResult(lotId, issueNo, drawRet,srcNo):   
    print "InsertQXCDrawResult begin!"
    dbInfo = dict()
    dbInfo = InitDB()    
    if(dbInfo.has_key("error")):
        return
    try:
        ...
        lot_cur.close()
        lot_conn.close()
        print "InsertQXCDrawResult SUCC for [%s %s %s] at %s" % (lotId,issueNo,drawRet['draw_result'],strftime("%Y-%m-%d %H:%M:%S",localtime(int(time()))))
        return 0
    except Exception, e:
        printExe("InsertQXCDrawResult FAIL for [%s %s %s]" % (lotId,issueNo,drawRet['draw_result']))
        return(-1)
...
if __name__ == "__main__":
    ...
    #         tmp = "0" + tmp
    #     issue_no = "20150" + tmp
    #     IssueDrawSnatch("PLW", issue_no, False)
    #     sleep(1)
    #     i = i + 1
